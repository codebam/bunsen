...
///
/// This is the one line this vendored copy exists for. Upstream blitz-net
/// hardcodes a Firefox 81 string from 2020 and appends it *after* any headers
/// the caller supplies, so a user agent set on a Request is discarded — there
/// is no way to override it from outside the crate. Enough of the web gates
/// on this that an ancient agent is actively harmful: sites serve degraded
/// markup or refuse outright.
///
/// `BUNSEN_USER_AGENT` overrides it, which is what a
/// request-desktop-site toggle would drive.
fn user_agent() -> String {
    std::env::var("BUNSEN_USER_AGENT").unwrap_or_else(|_| {
